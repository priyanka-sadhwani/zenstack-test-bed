...
exports.getDataModelAndTypeDefs = getDataModelAndTypeDefs;
...
exports.getAuthDecl = getAuthDecl;
...
exports.getInheritedFromDelegate = getInheritedFromDelegate;
exports.getInheritanceChain = getInheritanceChain;
exports.getRelationBackLink = getRelationBackLink;
exports.getRelationName = getRelationName;
exports.getPrismaClientGenerator = getPrismaClientGenerator;
...
function getDataModelAndTypeDefs(model, includeIgnored = false) {
    const r = model.declarations.filter((d) => (0, ast_1.isDataModel)(d) || (0, ast_1.isTypeDef)(d));
    if (includeIgnored) {
        return r;
    }
    else {
...
function getPreviewFeatures(model) {
    const jsGenerator = model.declarations.find((d) => (0, ast_1.isGeneratorDecl)(d) &&
        d.fields.some((f) => f.name === 'provider' &&
            (getLiteral(f.value) === 'prisma-client-js' ||
                getLiteral(f.value) === 'prisma-client')));
    if (jsGenerator) {
...
function isDiscriminatorField(field) {
    var _a, _b;
    if (!(0, ast_1.isDataModel)(field.$container)) {
        return false;
    }
...
function getIdFields(decl) {
    const fields = (0, ast_1.isDataModel)(decl) ? getModelFieldsWithBases(decl) : decl.fields;
    ...
    if (fieldLevelId) {
        ...
        const modelIdAttr = decl.attributes.find((attr) => { var _a, _b; return ((_b = (_a = attr.decl) === null || _a === void 0 ? void 0 : _a.ref) === null || _b === void 0 ? void 0 : _b.name) === '@@id'; });
        if (modelIdAttr) {
            ...
            return argValue.items
                .filter((expr) => (0, ast_1.isReferenceExpr)(expr) && !!getFieldReference(expr))
...
function getFieldReference(expr) {
    if ((0, ast_1.isReferenceExpr)(expr) && ((0, ast_1.isDataModelField)(expr.target.ref) || (0, ast_1.isTypeDefField)(expr.target.ref))) {
        return expr.target.ref;
    }
    else if ((0, ast_1.isMemberAccessExpr)(expr) && ((0, ast_1.isDataModelField)(expr.member.ref) || (0, ast_1.isTypeDefField)(expr.member.ref))) {
...
function getRecursiveBases(dataModel, includeDelegate = true, seen = new Set()) {
    const result = [];
 ...
 */
function getInheritedFromDelegate(field) {
    ...
    const bases = getRecursiveBases(field.$container, true);
 ...
 */
function getInheritanceChain(from, to) {
    if (from === to) {
        return [];
    }
 ...
 */
function getRelationBackLink(field) {
    var _a, _b, _c;
    ...
    if (field.$inheritedFrom && isDelegateModel(field.$inheritedFrom)) {
        ...
        sourceModel = field.$container;
    }
    const targetModel = field.type.reference.ref;
    const sameField = (f1, f2) => {
        var _a, _b;
        // for fields inherited from a delegate model, always use
        // the base to compare
        const parent1 = (_a = f1.$inheritedFrom) !== null && _a !== void 0 ? _a : f1.$container;
        const parent2 = (_b = f2.$inheritedFrom) !== null && _b !== void 0 ? _b : f2.$container;
...
function getPrismaClientGenerator(model) {
    var _a, _b, _c;
    const decl = model.declarations.find((d) => (0, ast_1.isGeneratorDecl)(d) &&
        d.fields.some((f) => f.name === 'provider' &&
            (getLiteral(f.value) === 'prisma-client-js' ||
                getLiteral(f.value) === 'prisma-client')));
    if (!decl) {
    ...
    const provider = getLiteral((_a = decl.fields.find((f) => f.name === 'provider')) === null || _a === void 0 ? void 0 : _a.value);
    return {
        name: decl.name,
        output: getLiteral((_b = decl.fields.find((f) => f.name === 'output')) === null || _b === void 0 ? void 0 : _b.value),
        previewFeatures: getLiteralArray((_c = decl.fields.find((f) => f.name === 'previewFeatures')) === null || _c === void 0 ? void 0 : _c.value),
        provider,
