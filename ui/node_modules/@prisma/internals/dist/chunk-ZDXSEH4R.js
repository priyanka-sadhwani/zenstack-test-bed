...
var chunk_ZDXSEH4R_exports = {};
__export(chunk_ZDXSEH4R_exports, {
...
module.exports = __toCommonJS(chunk_ZDXSEH4R_exports);
...
var import_chunk_4NEPCR2S = require("./chunk-4NEPCR2S.js");
var import_chunk_ONV5CEYB = require("./chunk-ONV5CEYB.js");
var import_chunk_BJOGHK2K = require("./chunk-BJOGHK2K.js");
var import_chunk_ZBY5I5ER = require("./chunk-ZBY5I5ER.js");
var import_chunk_EYVG6XQS = require("./chunk-EYVG6XQS.js");
var import_chunk_JTIZ3F5J = require("./chunk-JTIZ3F5J.js");
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_JTIZ3F5J.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_BJOGHK2K.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_BJOGHK2K.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_JTIZ3F5J.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_4NEPCR2S.getConfig)({ datamodel: schemaResult.schemas, ignoreEnvVarErrors });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_ONV5CEYB.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_ZBY5I5ER.mergeSchemas)({ schemas: schemaContext.schemaFiles });
    ...
    for (const generator of generators) {
      if (generator.manifest && generator.manifest.requiresEngines) {
        ...
        if (engineVersion !== version2 && generator.options && generator.manifest.requiresEngines.includes(queryEngineType) && generatorBinaryPaths[queryEngineType] && generatorBinaryPaths[queryEngineType]?.[binaryTarget]) {
          const customDmmf = await (0, import_chunk_ONV5CEYB.getDMMF)({
            datamodel: schemaContext.schemaFiles,
            previewFeatures
          });
...
function checkUnsupportedDataProxy({
  ...
  const url = (0, import_chunk_4NEPCR2S.resolveUrl)((0, import_chunk_4NEPCR2S.getEffectiveUrl)(schemaContext.primaryDatasource));
