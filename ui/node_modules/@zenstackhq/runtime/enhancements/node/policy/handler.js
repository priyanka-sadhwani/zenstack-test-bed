...
class PolicyProxyHandler {
    ...
    validateCreateInputSchema(model, data) {
        ...
        return this.policyUtils.validateZodSchema(model, 'create', data, false, (err) => {
            throw this.policyUtils.deniedByPolicy(model, 'create', `input failed validation: ${(0, local_helpers_1.getZodErrorMessage)(err)}`, constants_1.CrudFailureReason.DATA_VALIDATION_VIOLATION, err);
        });
    }
    createMany(args) {
        if (!args) {
    ...
    validateUpdateInputSchema(model, data) {
        ...
        const validatedData = this.policyUtils.validateZodSchema(model, 'update', literalData, false, (err) => {
            throw this.policyUtils.deniedByPolicy(model, 'update', `input failed validation: ${(0, local_helpers_1.getZodErrorMessage)(err)}`, constants_1.CrudFailureReason.DATA_VALIDATION_VIOLATION, err);
        });
