  ...
  const queryOptions = computed(() => {
    return __spreadValues({
      ...
      queryFn: ({ queryKey: queryKey2, signal }) => {
        ...
        const reqUrl = makeUrl(url, toValue(args2));
        return fetcher(reqUrl, { signal }, fetch2, false);
      }
    }, optionsValue);
  });
  return __spreadValues({ queryKey }, useQuery(queryOptions));
}
function useInfiniteModelQuery(model, url, args, options, fetch2) {
  ...
  const queryOptions = computed(() => __spreadValues({
    queryKey,
    queryFn: ({ queryKey: queryKey2, pageParam, signal }) => {
      const [_prefix, _model, _op, args2] = queryKey2;
      const reqUrl = makeUrl(url, pageParam != null ? pageParam : toValue(args2));
      return fetcher(reqUrl, { signal }, fetch2, false);
    },
    initialPageParam: toValue(args)
  }, toValue(options)));
  return __spreadValues({ queryKey }, useInfiniteQuery(queryOptions));
}
function useModelMutation(model, method, url, modelMeta, options, fetch2, checkReadBack) {
  ...
  const optionsValue = toValue(options);
  const finalOptions = computed(() => __spreadProps(__spreadValues({}, optionsValue), { mutationFn }));
